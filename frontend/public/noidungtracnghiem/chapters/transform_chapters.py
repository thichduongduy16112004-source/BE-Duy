...
def transform_question(question, topic, global_id):
    """Transform a single question to standard format"""
    # Convert answer index to full string
    answer_index = question["answer"]
    if not isinstance(answer_index, int) or answer_index < 0 or answer_index >= len(question["options"]):
        print(f"[WARN] Invalid answer index {answer_index} for question {question['id']}")
        answer_text = question["options"][0]  # Fallback to first option
    else:
...
def transform_topic(topic_data, global_id_start):
    ...
    topic = {
        ...
        "lessonNodes": topic_data["lessonNodes"],
        "questions": []
    }
    ...
    for question in topic_data["questions"]:
        transformed_q = transform_question(question, topic, global_id)
